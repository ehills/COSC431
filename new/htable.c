...
htable htable_new(int capacity, hashing_t hashing_type){
    ...
    result->num_keys = 0;
    result->keys = emalloc(capacity * sizeof(key_pos_pair));
...
int htable_insert(htable h, const char *s, long docid){
    ...
    for (collisions = 0; collisions <= h->capacity; collisions++){
        if (h->keys[position] == NULL) {
            h->keys[position] = kpp_new();
            kpp_add(h->keys[position] ,s ,docid);
            h->num_keys++;
            return 1;
        } else if (!strcmp(kpp_get_key(h->keys[position]), s)){
            kpp_add(h->keys[position], s, docid);
            return 1;
        } else {
...
int htable_search(htable h, const char *s){
    ...
    for (i = 0; i <= h->capacity; i++){
        if (h->keys[position] == NULL){
            return 0;
        } else if(strcmp(kpp_get_key(h->keys[position]), s) == 0){
            return 1;
        } else {
            position = (position + step) % h->capacity;
        }
...
int htable_save_to_disk(htable h, FILE* fp) {
    ...
    for (i = 0; i <= h->capacity; i++) {
        if (h->keys[i] != NULL) {
            pos += kpp_save_to_disk(h->keys[i], postings_fp, fp); // need to think abit
        }
    }
...
void htable_print(htable h){
    ...
    for (i = 0; i < h->capacity; i++){
        if (h->keys[i] != NULL) {
            printf("%s\t", kpp_get_key(h->keys[i]));
            kpp_print_postings(h->keys[i]);
            printf("\n");
        }
    }
